...
- a relation column is typed by the column it points at, not by the relation itself, so a foreign
  key to a `BigAutoField` primary key is rendered as `big_auto`
